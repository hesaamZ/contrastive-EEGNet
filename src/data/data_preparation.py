...
def get_data(path, save_path, long = False, normalization = 0,subjects_list=range(1,105), n_classes=4, type_run='imagery'):
    '''
    ...
    if type_run == "imagery":
        ...
        type_name = '-imagined-run_'
    elif type_run == "execution":
        mi_runs = [3, 5, 7, 9, 11, 13]
        type_name = '-execution-run_'

    # Extract only requested number of classes
    if (n_classes == 3 and type_run == "imagery"):
        print('Returning 3 Class data')
        mi_runs.remove(6)  # feet
        mi_runs.remove(10)  # feet
        mi_runs.remove(14)  # feet
    elif (n_classes == 3 and type_run == "execution"):
        print('Returning 3 Class data')
        mi_runs.remove(5)  # feet
        mi_runs.remove(9)  # feet
        mi_runs.remove(13)  # feet
    elif (n_classes == 2 and type_run == "imagery"):
        print('Returning 2 Class data')
        mi_runs.remove(6)  # feet
        mi_runs.remove(10)  # feet
        mi_runs.remove(14)  # feet
        mi_runs.remove(1)  # rest
    elif (n_classes == 2 and type_run == "execution"):
        print('Returning 2 Class data')
        mi_runs.remove(5)  # feet
        mi_runs.remove(9)  # feet
        mi_runs.remove(13)  # feet
    ...
    read_data(subjects = subjects,runs = mi_runs, path=path, long=long, save_path=save_path, type_name=type_name)
...
def read_data(subjects , runs, path, save_path, type_name, long=False):
    '''
    Load data samples and return it as one array
    Parameters:
    -----------
            ...
            if run in second_set:
                for ii in range(0,np.size(labels)):
                    if(labels[ii] == 'T0' and counter_0 < NO_trials):
                        ...
                        temp = np.array(sigbufs[:,int(points[ii]):int(points[ii])+n_samples])[None]
                        name = str(subject) + type_name + str(run) + '-' + str(ii) + '-' + 'feet.npy'
                        file_name = os.path.join(save_path, name)
                        np.save(file_name, temp)
                        data_step = np.vstack((data_step, temp))
                ...
                for ii in range(0,np.size(labels)):
                    if(labels[ii] == 'T0' and counter_0 < NO_trials):
                        ...
                        temp = np.array(sigbufs[:,int(points[ii]):int(points[ii])+n_samples])[None]
                        name = str(subject) + type_name + str(run) + '-' + str(ii) + '-' + 'left_hand.npy'
                        file_name = os.path.join(save_path, name)
                        np.save(file_name, temp)
                        data_step = np.vstack((data_step, temp))
                        ...
                        temp = np.array(sigbufs[:,int(points[ii]):int(points[ii])+n_samples])[None]
                        name =  str(subject) + type_name + str(run) + '-' + str(ii) + '-' + 'right_hand.npy'
                        file_name = os.path.join(save_path, name)
                        np.save(file_name, temp)
                        data_step = np.vstack((data_step, temp))
                ...
                for ii in range(0,20):
                    if(counter_0 < 20):
                        counter_0 += 1
                        labels_int = np.append(labels_int, [2])
                        temp = np.array(sigbufs[:,(ii*n_samples):((ii+1)*n_samples)])[None]
                        name = str(subject) + type_name + str(run) + '-' + str(ii) + '-' + 'rest.npy'
                        file_name = os.path.join(save_path, name)
                        np.save(file_name, temp)
                        data_step = np.vstack((data_step, temp))
